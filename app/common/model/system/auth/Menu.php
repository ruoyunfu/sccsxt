...
class Menu extends BaseModel
{
    ...
    {
        return $this->hasOne(self::class, 'menu_id', 'pid');
    }
    ...
    {
        return self::where('menu_id',$this->pid)->find();
    }

    public function getChildAttr($value,$data)
    {
        return self::where(['pid'=>$data['menu_id'],'is_menu'=>1])->find();
    }

    public function getParentsAttr()
    {
        $menu_ids = explode('/',$this->path);
        return self::whereIn('menu_id',$menu_ids)->select()->toArray();
    }
