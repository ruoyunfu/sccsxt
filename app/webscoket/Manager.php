...
class Manager extends Websocket
{
    ...
    public function onMessage(Frame $frame)
    {
        ...
        if ($result['type'] == 'data_status') {
            $k = env('APP_KEY','merchant').'_data_status';
            Cache::set($k, time(),60);
            $data = app()->make(DataScreenRepository::class)->data_screen();
            return $this->send($frame->fd,  app('json')->message('data_screen', $data));
        }
        $data = $result['data'] ?? [];
        $frame->uid = $info['uid'];
