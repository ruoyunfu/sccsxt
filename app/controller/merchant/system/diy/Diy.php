...
class Diy extends BaseController
{
    ...
    public function saveData(int $id = 0)
    {
        ...
        if ($infoDiy) {
            if(in_array($infoDiy['is_diy'], [DiyRepository::IS_DIY_DIY, DiyRepository::IS_DIY_MICRO])) {
...
//        }
    ...
    public function productLst()
    {
        ...
        $where['is_gift_bag'] = 0; // 不是礼包
        $where['keyword'] = $where['store_name']; // 关键字为店铺名称
        // 如果是商家管理员，则添加商家ID查询条件
        if ($this->request->merId()) $where['mer_id'] = $this->request->merId();
        // 调用 SpuRepository 类的 getApiSearch 方法获取商品列表
    ...
    {
        $data = $this->repository->productCategoryInfo((int)$this->request->merId());
        return app('json')->success($data);
    }
