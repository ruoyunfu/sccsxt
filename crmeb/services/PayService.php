...
class PayService
{
    ...
    public function payWeixinQr(?User $user)
    {
        ...
        return ['config' => $config['code_url'], 'time_expire' => time() + (15 * 60)];
    }
    ...
    public function payAlipayQr(? User $user)
    {
        ...
        return ['config' => $url, 'time_expire' => time() + (15 * 60)];
    }
    ...
    {
        $config = WechatService::create()->payWeixinBarCode($this->options['order_sn'], $this->options['pay_price'], $this->options['attach'], $this->options['body'], $this->options['auth_code'], $this->type);
        return $config;
    }

    public function payAlipayBarCode()
    {
        $config = AlipayService::create($this->affect)->payAlipayBarCode($this->options['order_sn'], $this->options['pay_price'], $this->options['body'], $this->options['auth_code'], $this->type);
        return $config;
    }
}
