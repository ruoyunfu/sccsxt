...
class Uupt extends BaseStorage implements DeliveryInterface
{
    ...
    public function addOrder($data)
    {
        $params = [
            ...
            'push_type' => $data['push_type'], //推送方式（0 开放订单，2测试订单）默认传0即可
            'special_type' => '0',
    ...
    public function cancelOrder($data)
    {
        $params = [
            ...
            'reason'    => $data['reason'] ? $data['reason'] : '用户取消',
        ];
    ...
    protected function getMessage($json, $message = '未知错误！')
    {
        ...
        if (!in_array($data['return_code'], ['ok', 'fail'])) {
            ...
            Log::info('【UU跑腿提示错误】:'.$mes.', 错误码:'.$data['return_code']);
            throw new ValidateException('【UU跑腿提示错误】:'.$mes);
        } else {
            return $data;
        }
    }
