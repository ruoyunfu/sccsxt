...
//            $this->authThumb && $this->thumb($this->fileInfo->filePath);
    ...
    {
        try {
            $res = $this->app()->listBuckets(systemConfig('jd_storageRegion'));
            return $res ?? [];
        } catch (\Throwable $e) {
            return [];
        }
...
//                    if (systemConfig('image_thumbnail_status') && isset($config[$height]) && isset($config[$width])) {
                    if (isset($config[$height]) && isset($config[$width])) {
                        if (!strpos($filePath, '?x-oss-process')) {
                            $this->fileInfo->$key = $filePath . '?x-oss-process=img/s/'.$config[$width].'/'
                            .$config[$height];
                        }
        ...
        if ($waterConfig['image_watermark_status'] && $filePath) {
            ...
            switch ($waterConfig['watermark_type']) {
                case 1://图片
                    ...
                    //京东入参格式：桶名:filename.jpg
                    $waterFileName = substr(parse_url($waterConfig['watermark_image'],PHP_URL_PATH), 1);
                    $waterImage = $this->storageName.':'.$waterFileName;
                    $waterPath = $filePath .= '/wmi/wk/' . base64_encode($waterImage) . '/wd/' . $waterConfig['watermark_opacity'] . '/wp/' . ($waterConfig['watermark_position'] ?? '5') . '/wdx/' . $waterConfig['watermark_x'] . '/wdy/' . $waterConfig['watermark_y'];
                    break;
                case 2://文字
                    ...
                    $waterPath = $filePath .= '/wmt/wt/' . base64_encode($waterConfig['watermark_text']) . '/wc/_' . $waterConfig['watermark_text_color'] . '/ws/' . $waterConfig['watermark_text_size'] . '/wp/' . ($waterConfig['watermark_position'] ?? 1) . '/wdx/' . $waterConfig['watermark_x'] . '/wdy/' . $waterConfig['watermark_y'];
                    break;
            }
        }
        return $waterPath;
