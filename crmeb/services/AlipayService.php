...
//                'debitCardExpress',// 	借记卡快捷
    ...
    public function payAlipayBarCode($out_trade_no, $total_fee, $body, $auth_code, $type)
    {
        $data = [
            'trade_no' => $out_trade_no,
            'scene' => 'bar_code',
        ...
        $res = $this->application->pay(Client::ALI_CHANNEL_BAR, $data);
        ...
        if($res['code'] == '10000' && $res['msg'] == 'Success') {
            ...
            throw new ValidateException('需要用户输入支付密码');
        }
    ...
    public function query(string $trade_sn)
    {
        ...
        try {
            return $this->application->tradeQuery($data);
        } catch (\Exception $e) {
            throw new ValidateException('支付宝支付错误返回：' . $e->getMessage());
        }
    }
