...
class SmsService
{
    ...
    {
        $gateway = $gateway ?: ((int)systemConfig('sms_use_type') ?: 1);
        switch ($gateway) {
            case 1:
                $name = 'yunxin';
                $config = [
                    'account' => systemConfigNoCache('serve_account'),
                    'secret' => systemConfigNoCache('serve_token')
                ];
                break;
            case 2:
                $name = 'aliyun';
    ...
    public static function sendMessage($data)
    {
        ...
        switch ($tempId) {
            case 'ASSIST_SUCCESS' :
                $res = app()->make(ProductAssistSetRepository::class)->get($id);
                if (!$res->user || !$res->user->phone)  return true;
                self::create()->send($res->user->phone, $tempId, ['date' => $res->create_time]);
                break;
                //发货提醒 -2.1
            case 'DELIVER_GOODS_CODE':
                $order = app()->make(StoreOrderRepository::class)->get($id);
                if (!$order || !$order->user_phone) return true;
            ...
            case 'EXTRACT_NOTICE':
                ...
                if (!$res || !$res->user->phone) return true;
                $is_status = $params['is_status'] == 1 ?  '已通过' : '已驳回';
                $msg = ['store_name' => systemConfig('site_name'),'date' => date('Y-m-d H:i:s'),'number' => $res['extract_price'],'is_status' => $is_status];
                Log::info('EXTRACT_NOTICE:'.var_export([$msg,$data],1));
                self::create()->send($res->user->phone, $tempId,$msg);
                break;
            case 'TRANSFER_ACCOUNTS_SUCCESS':  // 平台审核转账结果通知
                $res = app()->make(FinancialRepository::class)->get($id);
                if (!$res || !$res->merchant->mer_phone) return;
                self::create()->send($res->merchant->mer_phone, $tempId, ['mer_name' => $res->merchant->mer_name, 'site_name' => systemConfig('site_name'), 'datetime' => date('Y年m月d日 H时i分'), 'amount' => $res->extract_money]);
                break;
        }
    }
