...
Route::group(function () {
    ...
    Route::get('change/color', 'admin.Common/getChangeColor')->name('merchantGetChangeColor')->option([
        '_alias' => '一键换色',
    ...
    Route::group('diy/productCategory', function () {
        Route::get('info', '/productCategoryInfo')->name('merchantDiyProductCategoryInfo')->option([
            '_alias' => '商品分类信息',
        ]);
        Route::post('create/:id', '/saveData')->name('merchantDiyProductCategorySave')->option([
            '_alias' => '保存商品分类',
        ]);
    })->prefix('merchant.system.diy.Diy')->option([
        '_path' => '/devise/diy/product_category',
        '_auth' => true
    ])->append(['is_diy' => 4, 'name' => '商户商品分类', 'title' => '商户商品分类']);
