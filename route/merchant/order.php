...
Route::group(function () {
    ...
    Route::group('store/order', function () {
        ...
        //获取快递信息
        Route::get('kuaidi_coms', 'Order/getKuaidiComs')->option(['real_name' => '获取快递信息']);
        //获取寄件价格
        Route::get('shipment/getPrice/:id', 'Order/getPrice')->name('merchantStoreOrderGetPrice')->option([
            '_alias' => '获取商家寄件价格',
        ]);
        //获取寄件价格
        Route::get('shipment/list', 'Order/shipmentList')->name('merchantStoreOrderShipmentList')->option([
            '_alias' => '获取商家寄件价格',
        ]);
        Route::post('shipment/cancel/:id', 'Order/cancelShipment')->name('merchantStoreOrderCancelShipment')
            ->option([
            '_alias' => '取消商家寄件',
        ]);
        Route::get('note', 'Order/note')->name('merchantStoreOrderNote')->option([
            '_alias' => '配货单',
        ]);
        ...
        Route::post('repeat_dump/:id', 'Order/repeatDump')->name('merchantStoreOrderRepeatDump')->option([
            '_alias' => '电子面单复打',
        ]);
        ...
        Route::get('collectCargo/:id/form', 'Order/collectCargoForm')->name('merchantStoreOrderCollectCargoForm')->option([
            '_alias' => '修改收货信息表单',
            '_auth' => false,
            '_form' => 'merchantStoreOrderCollectCargoForm',
        ]);
        Route::post('collectCargo/:id', 'Order/collectCargo')->name('merchantStoreOrderCollectCargo')->option([
            '_alias' => '修改收货信息',
        ]);
        ...
        Route::post('offline/:id', 'Order/offline')->name('merchantStoreOrderOffline')->option([
            '_alias' => '线下支付',
        ]);
        Route::group('reservation', function () {
            Route::post('dispatch/:id', 'Order/reservationDispatch')->name('merchantStoreOrderReservationDispatch')->option([
                '_alias' => '预约订单派单',
            ]);
            Route::post('updateDispatch/:id', 'Order/reservationUpdateDispatch')->name('merchantStoreOrderReservationUpdateDispatch')->option([
                '_alias' => '预约订单改派',
            ]);
            Route::post('reschedule/:id', 'Order/reservationReschedule')->name('merchantStoreOrderReservationReschedule')->option([
                '_alias' => '预约订单改约',
            ]);
            Route::post('reservationTime/:id', 'Order/reservationTime')->name('merchantStoreOrderReservationTime')->option([
                '_alias' => '单独修改预约时间',
            ]);
            Route::post('verify/:id', 'Order/reservationVerify')->name('merchantStoreOrderReservationVerify')->option([
                '_alias' => '预约订单核销',
            ]);
        });
        Route::group('delivery', function () {
            Route::post('dispatch/:id', 'Order/deliveryDispatch')->name('merchantStoreOrderDeliveryDispatch')->option([
                '_alias' => '同城配送派单',
            ]);
            Route::post('updateDispatch/:id', 'Order/deliveryUpdateDispatch')->name('merchantStoreOrderDeliveryUpdateDispatch')->option([
                '_alias' => '同城配送改派',
            ]);
            Route::post('confirm/:id', 'Order/deliveryConfirm')->name('merchantStoreOrderDeliveryConfirm')->option([
                '_alias' => '同城配送核销',
            ]);
        });
        Route::post('deliveryOrderSync', 'Order/deliveryOrderSync')->name('merchantStoreOrderDeliverySync')->option([
            '_alias' => '同城配送再次同步',
        ]);
    ...
    Route::group('reservation/service', function () {
        Route::get('list', 'ReservationService/list')->name('merchantReservationServiceList')->option([
            '_alias' => '预约日历',
            '_auth' => true,
        ]);;
    })->prefix('merchant.store.order.')->option([
        '_path' => '/order/reservation',
        '_auth' => true
    ]);
